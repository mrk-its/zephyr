...
def gen_offset_header_cmd(target_file, src_file):
    cmd = (
        get_python_exe(),
        '"%s"'
        % os.path.join(
    ...
    return env.Command(
        target_file,
        src_file,
        env.VerboseAction(" ".join(cmd), "Generating header file $TARGET"),
    )


def generate_heap_constants_header_cmd():
    heap_constants_header = os.path.join(
        "$BUILD_DIR",
        "zephyr",
        "include",
        "generated",
    ...
    heap_constants_obj = os.path.join(
        "$BUILD_DIR",
        "heap_constants",
        "zephyr",
        "lib",
        "heap",
        "heap_constants.c" + (".o" if not ZEPHYR_PRESERVE_OBJ_EXT else ".obj"),
    )

    return gen_offset_header_cmd(heap_constants_header, heap_constants_obj)


def generate_offsets_header_file_cmd():

    offsets_header_file = os.path.join(
        "$BUILD_DIR",
        "zephyr",
        "include",
        "generated",
    ...
    return gen_offset_header_cmd(offsets_header_file, offsets_obj_file)
...
heap_constants_header_file = generate_heap_constants_header_cmd()
offset_header_file = generate_offsets_header_file_cmd()
env.Depends(offset_header_file, heap_constants_header_file)
...
for target, target_config in target_configs.items():
    ...
    if target_config["name"] != "heap_constants":
        # heap_constants library is skipped to avoid a cycle in dependencies
        # as it's later used for generating the offset header file
...
env.Depends(offsets_lib[0].sources, heap_constants_header_file)
...
# linked at interim step and not visible in the final linker command
tfpsacrypto_lib_config = target_configs.get(
    "tfpsacrypto", {}
)
if tfpsacrypto_lib_config:
    tfpsacrypto_lib_deps = get_project_lib_deps(
        framework_modules_map, tfpsacrypto_lib_config
    )
    for lib_dep in tfpsacrypto_lib_deps.values():
        env.Depends(
            framework_modules_map[tfpsacrypto_lib_config["id"]]["lib_path"],
            lib_dep["lib_path"],
        )
